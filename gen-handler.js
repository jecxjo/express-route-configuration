  ...
  async (req, res, next) => {
    if (rt.config.validate) {
      ...
      if (rt.config.validate.params) {
        ...
        if (error !== undefined) {
          st.logger.error(`${rt.method} ${rt.path} Param Validation - ${error.details[0].message}`);
      ...
      if (rt.config.validate.query) {
        ...
        if (error !== undefined) {
          st.logger.error(`${rt.method} ${rt.path} Query Validation - ${error.details[0].message}`);
          return res.json({ error: error.details[0].message });
        }
        req.query = value;
      }
      ...
      if (st.options.parsePayload && rt.config.validate.payload) {
        ...
        if (error !== undefined) {
          st.logger.error(`${rt.method} ${rt.path} Payload Validation - ${error.details[0].message}`);
          return res.json({ error: error.details[0].message });
        }
        req.body = value;
      }
